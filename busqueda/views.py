...
def base(request): 
    ...
    if searchTerm:
        if filterOption == 'titulo':
            cancion = Cancion.objects.filter(titulo__icontains=searchTerm)  #cancion es un queryset del modelo Cancion (Base de datos)
            ...
            cancion = Cancion.objects.filter(
                Q(titulo__icontains=searchTerm) |
                Q(artista__icontains=searchTerm) |
                Q(genero__icontains=searchTerm) |
                Q(año_publicacion__icontains=searchTerm) |
                Q(emocion__icontains=searchTerm)
            )
    else:
